...
const dashboard = http.createServer(function (_req, res) {
    ...
    res.write("Xin chào cậu chủ Nguyễn Văn Sang");
    res.end();
});

dashboard.listen(process.env.port || 0);

logger("BOT đã khởi tạo thành công", "BOT đang khởi tạo");
...
function startBot(message) {
    (message) ? logger(message, "BOT đang khởi chạy") : "";
    ...
    child.on("error", function (error) {
        logger("An error occurred: " + JSON.stringify(error), "BOT đang khởi tạo");
    });
};
